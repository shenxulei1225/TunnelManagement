...
INSERT IGNORE INTO system_menu (
    ...
    '分类数据分析', 'system:category:query', 2, 10, @parent_id, 'category-analysis', 
    'data-analysis', 'system/category/SimpleDebug', 'CategorySimpleDebug',
    1, 1, 1, 0, '1', NOW(), '1', NOW(), 0
);

-- 获取刚创建的菜单ID
SET @analysis_menu_id = LAST_INSERT_ID();

-- 为超级管理员角色分配权限
INSERT IGNORE INTO system_role_menu (role_id, menu_id, creator, create_time, updater, update_time, deleted, tenant_id)
SELECT 
    1 as role_id,
    @analysis_menu_id as menu_id,
    '1' as creator,
    NOW() as create_time,
    '1' as updater,
    NOW() as update_time,
...
AND NOT EXISTS (
    ...
    WHERE role_id = 1 AND menu_id = @analysis_menu_id AND deleted = 0
);
