...
INSERT IGNORE INTO system_menu (
    ...
    '分类管理调试版', 'system:category:query', 2, 9, @parent_id, 'category-debug', 
    'bug', 'system/category/DebugIndex', 'SystemCategoryDebug',
    1, 1, 1, 0, '1', NOW(), '1', NOW(), 0
);

-- 获取刚创建的菜单ID
SET @debug_menu_id = LAST_INSERT_ID();

-- 为超级管理员角色分配权限
INSERT IGNORE INTO system_role_menu (role_id, menu_id, creator, create_time, updater, update_time, deleted, tenant_id)
SELECT 
    1 as role_id,
    @debug_menu_id as menu_id,
    '1' as creator,
    NOW() as create_time,
    '1' as updater,
    NOW() as update_time,
...
AND NOT EXISTS (
    ...
    WHERE role_id = 1 AND menu_id = @debug_menu_id AND deleted = 0
);
