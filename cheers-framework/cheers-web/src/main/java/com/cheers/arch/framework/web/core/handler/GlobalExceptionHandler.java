package com.cheers.arch.framework.web.core.handler;
...
import com.cheers.arch.framework.common.util.collection.SetUtils;
import com.cheers.arch.framework.common.util.json.JsonUtils;
import com.cheers.arch.framework.common.util.monitor.TracerUtils;
import com.cheers.arch.framework.common.util.servlet.ServletUtils;
import com.cheers.arch.framework.web.core.util.WebFrameworkUtils;
import com.cheers.arch.framework.common.biz.infra.logger.ApiErrorLogCommonApi;
import com.cheers.arch.framework.common.biz.infra.logger.dto.ApiErrorLogCreateReqDTO;
...
import static com.cheers.arch.framework.common.exception.enums.GlobalErrorCodeConstants.*;
...
public class GlobalExceptionHandler {
    ...
    public CommonResult<?> defaultExceptionHandler(HttpServletRequest req, Throwable ex) {
        // 情况一：处理表不存在的异常
        CommonResult<?> tableNotExists = handleTableNotExists(ex);
        if (tableNotExists != null) {
    ...
    private void createExceptionLog(HttpServletRequest req, Throwable e) {
        ...
        try {
            // 初始化 errorLog
            this.buildExceptionLog(errorLog, req, e);
            // 执行插入 errorLog
    ...
    private CommonResult<?> handleTableNotExists(Throwable ex) {
        ...
        // 1. 提取表名
        String tableName = message.substring(message.indexOf("'") + 1, message.lastIndexOf("'"));
        // 2. 判断表名是否符合
        if (tableName.startsWith("cheers_")) {
            // 2.1 查找对应的模块
            String moduleName;
            if (tableName.startsWith("cheers_system_")) {
                moduleName = "系统管理";
            } else if (tableName.startsWith("cheers_device_")) {
                moduleName = "设备管理";
            } else if (tableName.startsWith("cheers_dynamic_")) {
                moduleName = "动态业务";
            } else if (tableName.startsWith("cheers_uxd_")) {
                moduleName = "UX设计器";
            } else {
                return null;
            }
            ...
            return CommonResult.error(INTERNAL_SERVER_ERROR.getCode(),
                    StrUtil.format("请在数据库中，创建对应的{}表，表名为({})", moduleName, tableName));
        }
        return null;
    }
