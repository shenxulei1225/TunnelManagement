package com.cheers.arch.framework.common.util.date;
...
import com.cheers.arch.framework.common.enums.DateIntervalEnum;
...
public class LocalDateTimeUtils {
    ...
    public static List<LocalDateTime[]> getDateRangeList(LocalDateTime startTime,
                                                         ...
                                                         Integer interval) {
        // 1.1 找到枚举
        DateIntervalEnum intervalEnum = DateIntervalEnum.findByInterval(interval);
        ...
        switch (intervalEnum) {
            ...
            case MONTH:
                while (startTime.isBefore(endTime)) {
                    LocalDateTime endOfMonth = endOfMonth(startTime);
            ...
            case QUARTER:
                while (startTime.isBefore(endTime)) {
                    LocalDateTime endOfQuarter = startTime.with(startTime.getMonth().firstMonthOfQuarter())
                            .with(TemporalAdjusters.lastDayOfMonth()).with(LocalTime.MAX);
                    timeRanges.add(new LocalDateTime[]{startTime, endOfQuarter});
                    startTime = endOfQuarter.plusNanos(1);
                }
                break;
            case YEAR:
                while (startTime.isBefore(endTime)) {
                    LocalDateTime endOfYear = startTime.with(TemporalAdjusters.lastDayOfYear()).with(LocalTime.MAX);
    ...
    public static String formatDateRange(LocalDateTime startTime, LocalDateTime endTime, Integer interval) {
        ...
        return StrUtil.format("{} 至 {}", LocalDateTimeUtil.format(startTime, format),
                LocalDateTimeUtil.format(endTime, format));
    }
