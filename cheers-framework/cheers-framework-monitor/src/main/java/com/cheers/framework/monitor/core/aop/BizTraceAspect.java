package com.cheers.framework.monitor.core.aop;

import cn.hutool.core.map.MapUtil;
import cn.hutool.core.util.StrUtil;
import com.cheers.framework.monitor.core.annotation.BizTrace;
import com.cheers.framework.monitor.core.util.TracerUtils;
...
public class BizTraceAspect {
    ...
    private void setBizTag(Span span, ProceedingJoinPoint joinPoint, BizTrace trace) {
        try {
            Map<String, Object> result = parseExpressions(joinPoint, asList(trace.type(), trace.id()));
    ...
    private Map<String, Object> parseExpressions(ProceedingJoinPoint joinPoint, Iterable<String> expressions) {
        // 如果为空，则返回空
        if (expressions == null) {
            return new HashMap<>();
        }
        ...
        expressions.forEach(expression -> {
            ...
            Object value = parseExpression(joinPoint, expression);
    ...
    private Object parseExpression(ProceedingJoinPoint joinPoint, String expression) {
        try {
            ...
            log.error("[parseExpression][expression({}) 解析失败]", expression, ex);
            return null;
        }
    }
