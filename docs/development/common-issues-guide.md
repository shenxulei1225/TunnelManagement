...
### 0.1 数据库表设计必须包含tenant_id字段

#### ❌ 常见问题：缺失tenant_id导致数据不可见
...
**原因：**
项目采用多租户架构，所有业务表都需要tenant_id字段进行数据隔离，但开发时遗漏了这个字段。

**解决方案：**

```sql
-- ✅ 正确：为业务表添加tenant_id字段
...
-- 修复历史数据（谨慎操作，建议先备份）
UPDATE your_business_table SET tenant_id = 1 WHERE tenant_id = 0;
```

```java
// ✅ 正确：实体类继承TenantBaseDO
...
public class YourBusinessDO extends TenantBaseDO {
    ...
    // tenant_id字段由TenantBaseDO提供
}
```

### 0.2 Mock数据和测试数据租户ID问题

#### ❌ 常见问题：Mock数据默认tenant_id为0导致数据不可见
...
**原因：**
使用randomPojo等工具创建测试数据时，tenant_id默认为0，但系统查询时使用的是租户ID为1的上下文。
...
public void testCreateData() {
    YourBusinessDO mockData = randomPojo(YourBusinessDO.class);
    // mockData.getTenantId() == 0，但查询上下文为1
}
...
const apiRequest = async () => {
  const tenantId = getTenantId() || '1' // 默认租户ID
...
- [ ] 数据库表包含`tenant_id`字段
- [ ] 实体类继承`TenantBaseDO`
- [ ] 确认表在`TenantProperties.ignoreTables`配置

**开发中必检查：**
- [ ] Mock数据设置正确的`tenantId`
...
这份指导建议涵盖了项目开发中的主要问题点，按照这些规范开发可以显著减少后期维护成本。建议将这些内容纳入团队的开发流程中，并在代码审查时重点检查这些方面。 