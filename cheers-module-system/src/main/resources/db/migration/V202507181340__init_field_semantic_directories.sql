-- 初始化字段语义分类目录数据
-- 使用 cheers-directory 框架的 system_directory 表
-- 使用 INSERT IGNORE 避免重复插入冲突

-- 1. 测量类字段目录
INSERT IGNORE INTO system_directory (business_type, parent_id, name, code, description, icon, sort, status, creator, create_time, updater, update_time, deleted, tenant_id) 
...
-- 8. 时间类字段目录
INSERT IGNORE INTO system_directory (business_type, parent_id, name, code, description, icon, sort, status, creator, create_time, updater, update_time, deleted, tenant_id)
VALUES ('FIELD_SEMANTIC', 0, '时间类', 'temporal', '时间戳、日期、时刻等时间相关字段', 'icon-clock', 8, 1, 'system', NOW(), 'system', NOW(), 0, 0); 